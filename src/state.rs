//! An abstraction around [`AHashMap`], where the types are the keys.
...
type AnyMap = AHashMap<TypeId, Box<dyn Any + Send + Sync>>;
...
impl State {
    ...
    ///
    /// Returns `None` if the value was replaced instead of it being newly
    ...
    /// Update a value in the [`State`]. with a method
